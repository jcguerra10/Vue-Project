<template>
    ...
    <div class="header__container--profile bg-dark" >
        <button v-if="!haveUser" class="bg-dark-ligth" @click="handleModal('signup')">
            Sign up
        </button>
        <button v-if="!haveUser" class="bg-secondary" @click="handleModal('signin')">
            Sign in
        </button>
        <h2 v-if="haveUser">{{ user.user.email === undefined ? "" : user.user.email }}</h2>
        <button class="bg-dark-ligth" v-if="haveUser" @click="(e) => handleSignOut(e)"> Sign Out</button>
    </div>
    ...
    methods: {
        ...
        handleSubmit(e) {
            ...
            if ((this.email !== "", this.password !== "")) {
                if (this.isSignUp) {
                    this.usersStore.signUp({email:this.email, password:this.password});
                } else {
                    this.usersStore.signIn({email:this.email, password:this.password});
                }
                this.isOpen = false;
                this.email = "";
                this.password = "";
